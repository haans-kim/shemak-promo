...
// v9 피드백: 00:47 화면 빠름 → OPENER slowdown, 00:51 자막 삭제, 00:53 sync
const PHASES = {
  OPENER:    { start: 0.2,  end: 8.5 },  // 6.5 → 8.5 slowdown
  CEO_VIEW:  { start: 8.5,  end: 18.5, video: "videos/phase13_ceo_overview.webm", videoStartFrom: 0 },
  HR_HEAD:   { start: 18.5, end: 35.0, video: "videos/phase16_ilji_simulation.webm", videoStartFrom: 0 },
  TEAM_LEAD: { start: 35.0, end: 47.05, video: "videos/phase17_team_skill.webm", videoStartFrom: 0 },
};

// v9: "판단·추론·해결" 자막 삭제 (피드백 00:51)
const CUES: Cue[] = [
  { start: 9.0,  end: 13.5, text: "경영진 AI — 전사 목표·KPI 모니터링" },
  { start: 14.0, end: 18.0, text: "연말 실적 추정 → 수정해야 할 과제 제시" },
  { start: 19.0, end: 23.5, text: "HR 임원 AI — 보상 인상률 최적 배분" },
...
const OpenerPhase: React.FC = () => {
  ...
  // v9 피드백 00:53: agent 카드를 "경영진·HR임원·팀장" 나레이션 시점에 맞춤
  const AGENTS = [
    { role: "경영진",   focus: "전사 목표·KPI",     color: BRAND.colors.primary,    delay: 5.2 },
    { role: "HR 임원",  focus: "보상 인상률 배분",  color: BRAND.colors.accent,     delay: 5.8 },
    { role: "팀장",     focus: "1:1 면담 · KPI",    color: BRAND.colors.accentWarm, delay: 6.4 },
  ];
  // 매듭 풀림 애니메이션: 4~5s 사이 꼬인 선이 3가닥으로 풀려 각 에이전트 카드로 이어짐
  const knotProgress = interpolate(lf, [4 * fps, 5.5 * fps], [0, 1], { extrapolateLeft: "clamp", extrapolateRight: "clamp" });
  return (
    <AbsoluteFill style={{
      ...
      {/* 매듭 풀림 애니메이션 — 3가닥 SVG path */}
      <div style={{ position: "relative", height: 120, display: "flex", justifyContent: "center" }}>
        <svg width="600" height="120" viewBox="0 0 600 120" style={{ opacity: interpolate(lf, [3.5 * fps, 4 * fps, 6.5 * fps], [0, 1, 0], { extrapolateLeft: "clamp", extrapolateRight: "clamp" }) }}>
